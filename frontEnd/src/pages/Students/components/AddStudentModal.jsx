...
const AddStudentModal = ({ open, onCancel, onSubmit, loading }) => {
    ...
    const { clearAutoSave } = useFormAutoSave('add-student-form', watch, reset);
    ...
    useEffect(() => {
        ...
        if (open) {
            fetchInitialData();
            if (!localStorage.getItem('add-student-form')) {
                reset({
                    mssv: '', fullName: '', email: '', phone: '',
                    department: undefined, faculty: undefined, major: undefined, class: undefined,
                    dob: null, gender: 'Other', address: '',
                    status: 'Active', avatarUrl: null
                });
            }
    ...
    const onFormSubmit = async (data) => {
        ...
        if (success !== false) {
            ...
            reset({
                mssv: '', fullName: '', email: '', phone: '',
                department: undefined, major: undefined, class: undefined,
                dob: null, gender: 'Other', address: '',
                status: 'Active', avatarUrl: null
            });
        }
                    ...
                    <Row gutter={24}>
                        ...
                        <Col xs={24} md={8}>
                            <Form.Item label={t('students.id')} required validateStatus={errors.mssv ? 'error' : ''} help={<ErrorMessage error={errors.mssv?.message} />}>
                                <Controller name="mssv" control={control} render={({ field }) => <Input {...field} placeholder={t('students.idPlaceholder')} />} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
                            <Form.Item label={t('students.fullName')} required validateStatus={errors.fullName ? 'error' : ''} help={<ErrorMessage error={errors.fullName?.message} />}>
                                <Controller name="fullName" control={control} render={({ field }) => <Input {...field} placeholder={t('students.namePlaceholder')} />} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
                            <Form.Item label={t('students.email')} required validateStatus={errors.email ? 'error' : ''} help={<ErrorMessage error={errors.email?.message} />}>
                                <Controller name="email" control={control} render={({ field }) => <Input {...field} placeholder={t('students.emailPlaceholder')} />} />
                            </Form.Item>
                        </Col>

                        <Col xs={24} md={8}>
                            <Form.Item label={t('students.phone')} required validateStatus={errors.phone ? 'error' : ''} help={<ErrorMessage error={errors.phone?.message} />}>
                                <Controller name="phone" control={control} render={({ field }) => <Input {...field} placeholder={t('students.phonePlaceholder')} />} />
                            </Form.Item>
                        </Col>
                        <Col xs={24} md={8}>
