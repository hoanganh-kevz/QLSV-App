...
const ClassGradeSheetPage = () => {
    ...
    const columns = useMemo(() => {
        ...
        // Dynamically find all unique components across all grades
        const compsMap = new Map();
        filteredGrades.forEach(g => {
            if (g.components) {
                g.components.forEach(c => {
                    if (!compsMap.has(c.name)) compsMap.set(c.name, c.weight);
                });
            }
        });

        const compCols = Array.from(compsMap.keys()).map(name => ({
            title: `${name} (${compsMap.get(name)}%)`,
            key: name,
            width: 120,
            align: 'center',
            render: (_, r) => {
                 const comp = r.components && r.components.find(c => c.name === name);
                 return comp && comp.score != null ? comp.score : '—';
            }
        }));
        ...
        const tailCols = [
            {
                ...
                render: (v, r) => r.isExamBanned ? <Text strong style={{ color: '#f43f5e' }}>0</Text> : <Text strong style={{ color: v >= 5.0 ? 'var(--primary-color)' : '#ef4444' }}>{v ?? '—'}</Text>,
            },
            {
                title: t('grades.grade'), dataIndex: 'letterGrade', key: 'letterGrade', width: 100, align: 'center',
                render: (grade, r) => {
                    if (r.isExamBanned) return <Tag color="#f43f5e" style={{ borderRadius: '6px', fontWeight: 600 }}>CẤM THI (F)</Tag>;
                    return grade ? <Tag color={getGradeColor(grade)} style={{ borderRadius: '6px', fontWeight: 600 }}>{grade}</Tag> : '—';
                }
            }
        ];

        return [...baseCols, ...compCols, ...tailCols];
    }, [t, filteredGrades]);
    ...
    const handleExport = useCallback(async () => {
        ...
        const compNames = Array.from(compsMap.keys());

        const excelHeaders = [
            '#', t('students.mssv'), t('students.fullName'),
            ...compNames.map(name => `${name} (${compsMap.get(name)}%)`),
            t('grades.total'), t('grades.gpa4'), t('grades.grade')
        ];

        const exportData = filteredGrades.map((g, i) => {
            const row = [i + 1, g.studentMssv, g.studentName];
            compNames.forEach(name => {
                const comp = g.components && g.components.find(c => c.name === name);
                row.push(comp && comp.score != null ? comp.score : '—');
            });
            row.push(g.isExamBanned ? 0 : g.totalScore, g.isExamBanned ? 0.0 : g.gpa4, g.isExamBanned ? 'F' : g.letterGrade);
            return row;
        });
