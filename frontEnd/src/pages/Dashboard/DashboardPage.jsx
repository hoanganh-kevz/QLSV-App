...
import { Typography, Button, Space, Spin, Result, Select } from 'antd';
import { 
    BarChartOutlined,
    BellOutlined,
    RocketOutlined,
...
const DashboardPage = () => {
    ...
    useEffect(() => {
        const loadTerms = async () => {
            ...
            if (res.success) {
                setTerms(res.data);
                const active = res.data.find(t => t.status === 'Active' || t.isDefault);
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await dashboardService.getDashboardStats(selectedTerm);
                if (response.success) {
                ...
                <Space>
                    {(role === 'admin' || role === 'manager' || role === 'teacher') && (
                        <Select 
                            ...
                            allowClear
                        >
                            {terms.map(t => <Select.Option key={t._id} value={t._id}>{t.name}</Select.Option>)}
                        </Select>
                    )}
