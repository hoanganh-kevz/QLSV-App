...
const EditSubjectModal = ({ open, onCancel, onSubmit, loading, initialData, faculties = [] }) => {
    ...
    useEffect(() => {
        if (open && initialData) {
            reset({
                ...initialData,
                faculty: initialData.faculty ? (typeof initialData.faculty === 'object' ? initialData.faculty._id : initialData.faculty) : undefined
            });
        }
    ...
    return (
        <FormModal
            title={`${t('subjects.editTitle')}: ${initialData?.name || ''}`}
            open={open}
            onCancel={onCancel}
                ...
                <Row gutter={24}>
                    <Col xs={24} md={12}>
                        <Form.Item label={t('subjects.code')} required validateStatus={errors.code ? 'error' : ''} help={<ErrorMessage error={errors.code?.message} />}>
                            <Controller name="code" control={control} render={({ field }) => <Input {...field} placeholder="e.g. IT101" style={{ textTransform: 'uppercase' }} disabled />} />
                        </Form.Item>
                    </Col>
                    <Col xs={24} md={12}>
