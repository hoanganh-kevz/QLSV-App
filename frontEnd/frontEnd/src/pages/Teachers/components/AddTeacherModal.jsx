...
const AddTeacherModal = ({ open, onCancel, onSubmit, loading }) => {
    ...
    return (
        <FormModal title={t('teachers.addTitle')} open={open} onCancel={onCancel} onSubmit={handleSubmit(onSubmit)} loading={loading} width={780}>
                ...
                <Row gutter={24}>
                    ...
                    <Col xs={24} md={8}>
                        <Form.Item label={t('teachers.teacherId')} required validateStatus={errors.teacherId ? 'error' : ''} help={<ErrorMessage error={t(errors.teacherId?.message)} />}>
                            <Controller name="teacherId" control={control} render={({ field }) => <Input {...field} placeholder={t('teachers.teacherIdPlaceholder')} />} />
                        </Form.Item>
                    </Col>
                    <Col xs={24} md={8}>
                        <Form.Item label={t('teachers.fullName')} required validateStatus={errors.fullName ? 'error' : ''} help={<ErrorMessage error={t(errors.fullName?.message)} />}>
                            <Controller name="fullName" control={control} render={({ field }) => <Input {...field} placeholder={t('teachers.namePlaceholder')} />} />
                        </Form.Item>
                    </Col>
                    <Col xs={24} md={8}>
                        <Form.Item label={t('teachers.email')} required validateStatus={errors.email ? 'error' : ''} help={<ErrorMessage error={t(errors.email?.message)} />}>
                            <Controller name="email" control={control} render={({ field }) => <Input {...field} placeholder={t('teachers.emailPlaceholder')} />} />
                        </Form.Item>
                    </Col>
                    ...
                    <Col xs={24} md={8}>
                        <Form.Item label={t('teachers.phone')} required validateStatus={errors.phone ? 'error' : ''} help={<ErrorMessage error={t(errors.phone?.message)} />}>
                            <Controller name="phone" control={control} render={({ field }) => <Input {...field} placeholder={t('teachers.phonePlaceholder')} />} />
                        </Form.Item>
                    </Col>
                    <Col xs={24} md={8}>
                        <Form.Item label={t('teachers.gender')} required validateStatus={errors.gender ? 'error' : ''} help={<ErrorMessage error={t(errors.gender?.message)} />}>
                            <Controller name="gender" control={control} render={({ field }) => (
                                <Select {...field} placeholder={t('teachers.gender')}>
